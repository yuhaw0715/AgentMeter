...
public enum L10n {
    ...
    public static var fetchingQuota: String { isTraditionalChinese ? "正在抓取 Codex 額度…" : "Fetching Codex Quota…" }
    ...
    public static func localizedLimitName(_ name: String) -> String {
        ...
        if name.contains("5-Hour") || name.contains("5 Hour") {
            return "5 小時工作階段額度"
        }
    ...
    public static var cliMissingTitle: String { isTraditionalChinese ? "找不到 Codex CLI 執行檔" : "Codex CLI Not Found" }
    public static var notAuthTitle: String { isTraditionalChinese ? "需要登入 Codex CLI" : "Codex Login Required" }
