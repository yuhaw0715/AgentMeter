...
透過官方 Codex CLI 的 app-server 整合取得、解析並正規化即時 rate-limit 快照，無需自行快取憑證或呼叫私人端點。

## ADDED Requirements

### Requirement: 透過 JSON-RPC 連接 Codex app-server
系統 SHALL 啟動或連接官方 `codex app-server` 程序，並透過標準 stdio/socket 的 JSON-RPC 進行通訊。

#### Scenario: 成功呼叫 JSON-RPC
- **WHEN** AgentMeter 對執行中或可啟動的 `codex app-server` 發起重新整理
- **THEN** 系統執行 `account/rateLimits/read` 並取得有效的 rate-limit 回傳資料
...
系統 SHALL 動態解析 Provider 回傳的 limits，不硬編碼特定 limit 類型，並將其正規化為通用 limit 模型。

#### Scenario: 回傳標準 5 小時與每週額度
- **WHEN** Codex app-server 回傳標準 rate limits
- **THEN** 系統擷取 limit 識別碼、使用百分比與重置時間戳記至正規化模型中
